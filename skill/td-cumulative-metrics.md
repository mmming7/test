# ThinkingData 累计指标配置专家

为 ThinkingData 用户提供累计指标配置的完整指导方案，涵盖场景识别、实现路径推荐、SQL 代码生成和配置步骤说明。
...
- **场景 3**：其他分析平台的累计指标配置（非 ThinkingData）
- **场景 4**：累计指标的数据质量问题排查（应归数据质量 skill）

判断口诀：「ThinkingData + 累计计算 + 配置指导」
...
2. **实现路径推荐**：根据场景特点推荐最优实现方式（SQL、配置、组合方案）
3. **SQL 代码生成**：提供可直接使用的 SQL 查询代码
4. **配置步骤指导**：给出 ThinkingData 平台的详细配置步骤
...
#### 场景 1：小时展示累计
**特征**：需要在小时粒度下展示累计值（如当日 0 点到当前小时的累计）
**典型需求**：
- "今天 0-10 点的累计充值金额"
- "当日截至当前小时的累计新增用户"
